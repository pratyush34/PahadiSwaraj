...
async function connectWithFallback() {
  ...
  for (const uri of attempts) {
    ...
    try {
      ...
      console.log(
        ...
        uri.includes("127.0.0.1") ? "local instance" : "remote host",
      );
      dbConnected = true;
      return;
    } catch (err) {
      console.error(
        `MongoDB connection attempt failed for ${uri}:`,
        err.message || err,
      );
    }
  }
  ...
  console.error(
    "All MongoDB connection attempts failed. Continuing without DB connection.",
  );
  dbConnected = false;
}
...
function assembleVariant(
  ...
  const featureTag = featuresArray.length
    ? featuresArray.map((f) => `${f.trim()}`).join(", ")
    : "natural ingredients";
  const prepStyles = [
    "crafted with care",
...
function generateMockAICopy(
  ...
  for (let i = 0; i < 20; i += 1) {
    variants.push(
      assembleVariant(productName, ingredients, weight, featuresArray, tone, i),
    );
  }
  return variants;
}
...
app.post("/api/generate", verifyToken, async (req, res) => {
  try {
    if (mongoose.connection.readyState !== 1) {
      return res
        .status(503)
        .json({ error: "Database unavailable. Check MongoDB connection." });
    }
    ...
    // Construct a crisp engineered prompt using the user's inputs
    const promptText = `Write an engaging e-commerce product description for a product named "${productName}". 
    Ingredients: ${ingredients}. 
    Weight/Size: ${weight}. 
    Key Features: ${featuresArray.join(", ")}. 
    Write this description in a "${tone}" tone. Provide three completely distinct variations separated by the delimiter "---".`;

    // Make the asynchronous request to the Gemini API flash model
    // Around line 263 in backend/server.js
    const response = await ai.models.generateContent({
      model: "gemini-3.5-flash", // Update to the active production model configuration
      contents: promptText,
    });

    // Extract text and split by your chosen delimiter into an array
    const rawText = response.text || "";
    const generatedOutputs = rawText
      .split("---")
      .map((variant) => variant.trim())
      .filter(Boolean);
    const generatedOutput = generatedOutputs[0] || rawText;

    // -----------------------------------------------------

    // Save history point to MongoDB Atlas (Unchanged)
...
app.get("/api/history", verifyToken, async (req, res) => {
  try {
    ...
    const queryCondition = search
      ...
      : {};
    const history = await Description.find(queryCondition)
      .sort({ createdAt: -1 })
      .limit(10);
...
app.listen(PORT, () => {
  ...
  if (!dbConnected)
    console.warn(
      "Warning: Database is not connected. Routes that use the DB will return 503.",
    );
});
